...
def dcd(image, k=25):  # Increase the number of clusters to 13
    image = cv2.cvtColor(image, cv2.COLOR_LUV2BGR)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)  # Change to LAB color space
    ...
    for i in range(len(dominant_ratios)):
        color = list(dominant_colors[i] / 255.0) 
        ratio = dominant_ratios[i]
        feature.extend(color)
        feature.append(ratio)

    while len(feature) != 100:  # Adjust the length to match the new number of clusters
        feature.append(0)
    return feature
...
def extract_features(dataframe, feature_extractor):
    ...
    for image, label in zip(dataframe['image'], dataframe['label']):
        ...
        if feature_extractor == 'all_features':
            ...
            moment_features = extract_color_moment(image)
            cdc_features = dcd(image)
            ccv_features = ccv(image)
            features = np.concatenate([histogram_features, moment_features, cdc_features, ccv_features])
        else:
...
def main():
    ...
    accuracy_color_histogram = []
    accuracy_color_moment = []
    accuracy_dcd = []
    accuracy_ccv = []
    
    accuracy_results = {
        'color_histogram': accuracy_color_histogram,
        'color_moment': accuracy_color_moment,
        'dcd': accuracy_dcd,
        'ccv': accuracy_ccv,
        'all_features': []
    }
    
    # Train và test với từng loại đặc trưng
    for feature_name, extract_feature in extractor.items():
        ...
        for k in [1, 5]:
            for metric_name, metric in distance_metrics.items():
                ...
                accuracy_results[feature_name].append({
                    'k': k,
                    'metric': metric_name,
                    'accuracy': accuracy
                })
    ...
    # Hiển thị kết quả
    print('=========================================================================================')
    for feature_name, results in accuracy_results.items():
        print(f"Results for {feature_name}:")
        for result in results:
            print(f"k={result['k']}, metric={result['metric']}: accuracy={result['accuracy']}")
        print('-----------------------------------------')
